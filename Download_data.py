...
class CardsListApplication:
    ...
    def start_progress_and_save_data(self):
        ...
        self.progress_window = ProgressBarWindow(self.company_index,self.csv_file_name,"Transportujacy",on_progress_finish)
    ...
    def load_data_with_progress(self):
        ProgressBarWindow(self.company_index,self.csv_file_name,"Transportujacy", self.load_data_from_csv).run()
...
class CSVDataSaver:
    def __init__(self, progress_callback, csv_file, type):
        self.page_data = KpoRequests.KPO()
        self.progress_callback = progress_callback
        self.csv_file = csv_file
        self.type = type

    def api_call(self, page_number, type):

        if type == "transportujacy":
            return self.page_data.wyszukiwarka_kart_transportujacy(page_number, 50, False)
        elif type == "przekazujacy":
            return self.page_data.wyszukiwarka_kart(page_number, 50, False)
        elif type == "przejmujacy":
            return self.page_data.wyszukiwarka_kart_przejmujacy(page_number, 50, False)
        else:
            ...
            with open(self.csv_file, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                while hasNextPage:
                    ...
                    while retries < max_retries:
                        try:
                            page_data = self.api_call(self.page_number, self.type)
                            if page_data and 'items' in page_data and page_data['items']:
                                break
                            else:
                                raise Exception("Empty API response.")
                        except Exception as error:
                            print(f"Retry {retries + 1} of {max_retries} due to error: {error}")
                            retries += 1
...
class ProgressBarWindow:
    def __init__(self, company_index, csv_file, type, callback=None):
        self.callback = callback
        self.company_index = company_index
        self.root = tk.Tk()
        ...
        if type == "przekazujacy":
            initial_data = self.page_data.wyszukiwarka_kart(1, 50, True)
            self.elements = initial_data['totalPagesNumber']
        elif type == "transportujacy":
            initial_data = self.page_data.wyszukiwarka_kart_transportujacy(1, 50, True)
            self.elements = initial_data['totalPagesNumber']
        elif type == "przejmujacy":
            initial_data = self.page_data.wyszukiwarka_kart_przejmujacy(1, 50, True)
            self.elements = initial_data['totalPagesNumber']
        else:
            print("ERROR, Błędna funkcja")
    ...
    def start_data_saver(self):
        ...
        try:
            ...
            data_saver = CSVDataSaver(lambda current, total, status_text="": self.update_progress_bar(current, total, status_text), self.csv_file, "Transportujacy")
